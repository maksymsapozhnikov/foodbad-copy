...
class SessionParse extends Model
{
    protected $states = [];
    protected $platforms = [];
    ...
    public function parse($response, $address)
    {
        if (!empty($response) && is_array($response)) {
            ...
            Yii::error('There is no data. Provider: ' . $address, 'parse/api');
        }
        return true;
    }
    ...
    {
        try{
            $this->restaurant['platform_id'] = $this->setPlatform();
            $this->restaurant['state_id'] = $this->setState();
            $this->restaurant['suburb_id'] = $this->setSuburb($this->item['03 Suburb'], $this->restaurant['state_id']);
            $this->restaurant['title'] = $this->setTitle();
            $this->restaurant['rating'] = $this->setRating();
            $this->restaurant['delivery_fee'] = $this->setDeliveryFee();
            $this->restaurant['delivery_time'] = $this->setDeliveryTime();
            $this->restaurant['average_delivery_time'] = $this->setAverageDeliveryTime();
            $this->restaurant['image_link'] = $this->setImageLink();
            $this->restaurant['restaurant_suburb'] = !empty($this->item['11 Restaurant Suburb']) ? $this->setSuburb($this->item['11 Restaurant Suburb'], $this->restaurant['state_id']) : null;
            $this->restaurant['link'] = $this->setLink();
            $this->restaurant['clean_link'] = $this->setCleanLink();
            $this->restaurant['pre_order_times'] = $this->setPreOrderTimes();
            $this->restaurant['cuisine'] = $this->setCuisine();
            $this->restaurant['tags'] = $this->setCategories();
            return $this->restaurant;
        }catch (\Exception $e){
            \Yii::error(\json_encode($e), 'Parse data');
            return false;
        }
    }
    ...
    */
    ...
    {
        $string = $this->restaurant['delivery_time'];
        if (!empty($string)) {
    ...
    protected function setSuburb($title, $state_id)
    {
        ...
        throw new \Exception('Error: setSuburb - ' . __FILE__ . ':' . __LINE__);
    }
    ...
    protected function setState()
    {
        ...
        throw new \Exception('Error: setState - ' . __FILE__ . ':' . __LINE__);
    }
    ...
    protected function setRestaurant($data)
    {
        try{
            /* Update */
            ...
            Throw new \Exception('Error/Create Model: ' . \json_encode($model->errors));
        }catch (\Exception $e){
            \Yii::error($e, 'Update/create model');
            return false;
        }
